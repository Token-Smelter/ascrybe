...
//     context surface, src/runtime/plugin-context.mjs:357-360, which forwards
//     to src/runtime/plugin-context.mjs:37 `provide(type, version,
//     handler, pluginName)`. Real sites e.g.
//     plugins/session-notes/server/index.mjs:860,
//     plugins/approval-escrow/server/index.mjs:35,
//     plugins/work-dispatch/server/workTypes.mjs:1012.
//
//   binding (code)
//     `context.capabilities.require(type, versionRange)` —
//     src/runtime/plugin-context.mjs:361-363 forwarding to
//     capabilityRegistry.mjs:45. Real sites e.g.
//     plugins/task-goals/server/index.mjs:176-178,
//     plugins/project-index/server/index.mjs:114,:126,
//     plugins/work-dispatch/server/index.mjs:2500.
//
//   invocation (code)
...
//     site, e.g. plugins/task-goals/server/index.mjs:182,
//     plugins/workflow-engine/server/index.mjs:4271.
//
//   declaration (manifest)
//     `provides_capabilities:` / `requires_capabilities:` lists of
//     `{ type, version, optional? }` — the shape src/runtime/plugin-context.mjs:46
...
//     plugins/task-goals/plugin.yaml:82-91.
...
//     (a constant, a variable) — real: plugins/work-dispatch/server/
//     workOrderManagementCapability.mjs:11-15 passes
//     `WORK_ORDER_MANAGEMENT_CAPABILITY`, and plugins/work-browser/server/
//     index.mjs:71 + plugins/scoped-delivery/server/index.mjs:27 pass a
//     parameter;
...
//     line (`\n  .request({`) — real: plugins/workflow-engine/server/index.mjs:2637;
...
// that plugins/work-browser/server/index.mjs:71 really writes); what makes it a
...
// plugins/work-dispatch/server/index.mjs:5908-5912 really writes. The
...
// try/catch that plugins/task-goals/server/index.mjs:176-178 writes.
...
// (src/runtime/plugin-context.mjs:50). On an estate with no capability
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (CAPABILITY_CALL.test(line)) {
      ...
      if (!type) {
        ...
        // plugins/work-dispatch/server/index.mjs:5908.
      ...
      if (!type) {
        ...
        refuse(index + 1, direction,
          ...
          `src/runtime/plugin-context.mjs:37 records the PROVIDING plugin as \`pluginName\`, and '${estatePath}' has no 'plugins/<owner>/' segment, so the plugin this ${direction} belongs to is not derivable`,
