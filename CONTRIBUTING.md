# Contributing to Ascrybe

**Read this only if you are changing Ascrybe itself.** To *use* Ascrybe — map an estate, query
it, send a graph to someone — read [USAGE.md](USAGE.md) instead; nothing here applies to that.
...
Git. Initialize external custody with `ascrybe state init`; it uses
